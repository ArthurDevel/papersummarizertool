...
class PaperProcessorClient:
    ...
    async def process_paper_pdf(self, pdf_contents: bytes, paper_id: str | None = None, arxiv_id_or_url: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Derive and include summary metrics for consumers to avoid recomputation outside processor
        try:
            num_pages_val = len(pages)
        except Exception:
            num_pages_val = 0
        try:
            total_cost_val = float(usage_summary.get("total_cost", 0.0))
        except Exception:
            total_cost_val = None
        avg_cost_per_page_val: Optional[float] = None
        try:
            if total_cost_val is not None and num_pages_val > 0:
                avg_cost_per_page_val = total_cost_val / float(num_pages_val)
        except Exception:
            avg_cost_per_page_val = None

        final_result["num_pages"] = num_pages_val
        final_result["total_cost"] = total_cost_val
        final_result["avg_cost_per_page"] = avg_cost_per_page_val
